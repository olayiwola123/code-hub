...
export default function About() {
  ...
  const isInView = useInView(ref, { once: true }); // This will only animate once when it comes into view
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col items-center justify-center p-8 md:flex-row gap-12" ref={ref}>
      ...
      <div className="relative md:w-1/2 w-full flex items-center justify-center h-96 rounded-xl bg-white p-6 md:p-12 shadow-md overflow-hidden">
        ...
        <motion.div
          ...
          variants={doorVariant}
          initial="hidden"
          animate={isInView ? "visible" : "hidden"} // Trigger doorVariant when in view
        />

        {/* Main Content */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : {}} // Fade in content when in view
          transition={{ delay: 1, duration: 0.8 }}
          className="z-20 text-center"
        >
          <h2 className="text-3xl font-bold text-gray-800 mb-4">About Our Certificate Portal</h2>
